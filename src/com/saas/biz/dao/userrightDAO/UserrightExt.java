...
public class UserrightExt extends UserrightDAO
{
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("td_b_userright");
  ...
  public UserrightDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    UserrightDAO localUserrightDAO = new UserrightDAO();
    Field[] arrayOfField = localUserrightDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localUserrightDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localUserrightDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "td_b_userright");
 ...
 * Qualified Name:     com.saas.biz.dao.userrightDAO.UserrightExt
