...
public class HistoricalsitesExt extends HistoricalsitesDAO
{
  HistoricalsitesDAO historicalsitesExt = new HistoricalsitesDAO();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("historicalsites");
  ...
  public HistoricalsitesDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    HistoricalsitesDAO localHistoricalsitesDAO = new HistoricalsitesDAO();
    Field[] arrayOfField = localHistoricalsitesDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localHistoricalsitesDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localHistoricalsitesDAO, arrayOfField[i].getName(), localInteger);
            }
        }
    ...
    return localHistoricalsitesDAO;
  }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "historicalsites");
 ...
 * Qualified Name:     com.saas.biz.dao.historicalsitesDAO.HistoricalsitesExt
