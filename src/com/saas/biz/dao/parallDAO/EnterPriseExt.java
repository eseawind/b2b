...
public class EnterPriseExt extends EnterPriseDAO
{
  ...
  public EnterPriseDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    EnterPriseDAO localEnterPriseDAO = new EnterPriseDAO();
    ArrayList localArrayList = new ArrayList();
    ...
    if (localArrayList.size() <= 0)
      return null;
    Field[] arrayOfField = localEnterPriseDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localEnterPriseDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localEnterPriseDAO, arrayOfField[i].getName(), localInteger);
            }
        }
 ...
 * Qualified Name:     com.saas.biz.dao.parallDAO.EnterPriseExt
