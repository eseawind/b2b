...
public class BidExt extends BidDAO
{
  ...
  public BidDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    BidDAO localBidDAO = new BidDAO();
    ArrayList localArrayList = new ArrayList();
    ...
    if (localArrayList.size() <= 0)
      return null;
    Field[] arrayOfField = localBidDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localBidDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localBidDAO, arrayOfField[i].getName(), localInteger);
            }
        }
 ...
 * Qualified Name:     com.saas.biz.dao.parallDAO.BidExt
