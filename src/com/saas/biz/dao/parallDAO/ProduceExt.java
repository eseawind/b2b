...
public class ProduceExt extends ProduceDAO
{
  ...
  public ProduceDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ProduceDAO localProduceDAO = new ProduceDAO();
    ArrayList localArrayList = new ArrayList();
    ...
    if (localArrayList.size() <= 0)
      return null;
    Field[] arrayOfField = localProduceDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localProduceDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localProduceDAO, arrayOfField[i].getName(), localInteger);
            }
        }
 ...
 * Qualified Name:     com.saas.biz.dao.parallDAO.ProduceExt
