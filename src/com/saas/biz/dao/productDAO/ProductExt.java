...
public class ProductExt extends ProductDAO
{
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("tf_f_product");
  ...
  public ProductDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    Field[] arrayOfField = localProductDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localProductDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localProductDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList selByListSql(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ArrayList localArrayList = new ArrayList();
    localArrayList = localDbexecute.selBizQuerySql(paramString);
    if (localArrayList.size() <= 0)
      return null;
    return localArrayList;
  }

  public ArrayList selByListSql(String paramString, int paramInt1, int paramInt2)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ArrayList localArrayList = new ArrayList();
    localArrayList = localDbexecute.selBizQuerySql(paramInt1, paramInt2, paramString);
    if (localArrayList.size() <= 0)
      return null;
    return localArrayList;
  }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "tf_f_product");
 ...
 * Qualified Name:     com.saas.biz.dao.productDAO.ProductExt
