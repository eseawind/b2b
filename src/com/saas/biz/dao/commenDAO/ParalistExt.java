...
public class ParalistExt extends ParalistDAO
{
  ...
  public ParalistDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ParalistDAO localParalistDAO = new ParalistDAO();
    ArrayList localArrayList = new ArrayList();
    localDbexecute.setStrTable("TD_S_DEPART");
    localDbexecute.setStrQuery(paramString);
    localDbexecute.setParamList(this.paramArray);
    ...
    if (localArrayList.size() <= 0)
      return null;
    Field[] arrayOfField = localParalistDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localParalistDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localParalistDAO, arrayOfField[i].getName(), localInteger);
            }
        }
 ...
 * Qualified Name:     com.saas.biz.dao.commenDAO.ParalistExt
