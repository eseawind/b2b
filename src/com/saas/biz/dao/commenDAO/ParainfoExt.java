...
public class ParainfoExt extends ParainfoDAO
{
  ...
  public ParainfoDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ParainfoDAO localParainfoDAO = new ParainfoDAO();
    ArrayList localArrayList = new ArrayList();
    localDbexecute.setStrTable("tf_f_customer");
    localDbexecute.setStrQuery(paramString);
    localDbexecute.setParamList(this.paramArray);
    ...
    if (localArrayList.size() <= 0)
      return null;
    Field[] arrayOfField = localParainfoDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localParainfoDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localParainfoDAO, arrayOfField[i].getName(), localInteger);
            }
        }
 ...
 * Qualified Name:     com.saas.biz.dao.commenDAO.ParainfoExt
