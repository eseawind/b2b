package com.saas.biz.dao.newproductattrDAO;
...
public class NewProductattrExt extends NewProductAttrDAO
{
  NewProductAttrDAO productattrDAO = new NewProductAttrDAO();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("tf_f_productattr");
  ...
  public NewProductAttrDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    Field[] arrayOfField = localNewProductAttrDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localNewProductAttrDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localNewProductAttrDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "tf_f_productattr");
 ...
 * Qualified Name:     com.saas.biz.dao.newproductattrDAO.NewProductattrExt
