package com.saas.biz.dao.condelDAO;
...
public class CondelExt extends CondelDAO
{
  CondelDAO condelDao = new CondelDAO();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("tf_f_condel");
  ...
  public CondelDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    CondelDAO localCondelDAO = new CondelDAO();
    Field[] arrayOfField = localCondelDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localCondelDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localCondelDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "tf_f_condel");
 ...
 * Qualified Name:     com.saas.biz.dao.condelDAO.CondelExt
