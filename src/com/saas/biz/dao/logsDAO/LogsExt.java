...
public class LogsExt extends LogsDAO
{
  LogsExt logsDao = new LogsExt();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("logs");
  ...
  public LogsExt selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    LogsExt localLogsExt = new LogsExt();
    Field[] arrayOfField = localLogsExt.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localLogsExt, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localLogsExt, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "logs");
 ...
 * Qualified Name:     com.saas.biz.dao.logsDAO.LogsExt
