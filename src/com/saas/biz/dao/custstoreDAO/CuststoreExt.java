package com.saas.biz.dao.custstoreDAO;
...
public class CuststoreExt extends CuststoreDAO
{
  CuststoreDAO custstoreDAO = new CuststoreDAO();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("td_b_custstore");
  ...
  public CuststoreDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    CuststoreDAO localCuststoreDAO = new CuststoreDAO();
    Field[] arrayOfField = localCuststoreDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localCuststoreDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localCuststoreDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "td_b_custstore");
 ...
 * Qualified Name:     com.saas.biz.dao.custstoreDAO.CuststoreExt
