...
public class UserauthxExt extends UserauthxDAO
{
  UserauthxDAO userauthxDao = new UserauthxDAO();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("userauthx");
  ...
  public UserauthxDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    UserauthxDAO localUserauthxDAO = new UserauthxDAO();
    Field[] arrayOfField = localUserauthxDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localUserauthxDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localUserauthxDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "userauthx");
 ...
 * Qualified Name:     com.saas.biz.dao.userauthxDAO.UserauthxExt
