...
public class PatentsExt extends PatentsDAO
{
  PatentsDAO patentsDao = new PatentsDAO();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("patents");
  ...
  public PatentsDAO selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    ...
    if (localArrayList.size() <= 0)
      return null;
    PatentsDAO localPatentsDAO = new PatentsDAO();
    Field[] arrayOfField = localPatentsDAO.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localPatentsDAO, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localPatentsDAO, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "patents");
 ...
 * Qualified Name:     com.saas.biz.dao.patentsDAO.PatentsExt
