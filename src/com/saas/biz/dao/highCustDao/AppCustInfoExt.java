package com.saas.biz.dao.highCustDao;
...
public class AppCustInfoExt extends AppCustInfoDao
{
  AppCustInfoDao appCustInfoDao = new AppCustInfoDao();
  ...
  public ArrayList selByList(String paramString)
  {
    ...
    localDbexecute.setStrTable("tf_f_app_highcust");
  ...
  public AppCustInfoDao selByInfo(String paramString)
  {
    Dbexecute localDbexecute = new Dbexecute();
    commMethodMgr localcommMethodMgr = new commMethodMgr();
    Logger localLogger = new Logger(this);
    ArrayList localArrayList = new ArrayList();
    localDbexecute.setStrTable("tf_b_group");
    localDbexecute.setStrQuery(paramString);
    localDbexecute.setParamList(this.paramArray);
    localArrayList = localDbexecute.selBizQuery();
    if (localArrayList.size() <= 0)
    ...
    Field[] arrayOfField = localAppCustInfoDao.getClass().getDeclaredFields();
      ...
      for (int i = 0; i <= arrayOfField.length - 1; i++)
      {
        ...
        try
        {
          if (localHashMap.get(arrayOfField[i].getName()) != null)
            ...
            {
              PropertyUtils.setProperty(localAppCustInfoDao, arrayOfField[i].getName(), localHashMap.get(arrayOfField[i].getName()).toString());
            }
            else if (arrayOfField[i].getType().getName() == "java.lang.Integer")
            {
              ...
              PropertyUtils.setProperty(localAppCustInfoDao, arrayOfField[i].getName(), localInteger);
            }
        }
  ...
  public ArrayList insBy(String paramString)
  {
    ArrayList localArrayList = new ArrayList();
    HashMap localHashMap = new HashMap();
    localHashMap.put("queryTable", "tf_f_app_highcust");
 ...
 * Qualified Name:     com.saas.biz.dao.highCustDao.AppCustInfoExt
