...
<%
  ...
  newsList = enquiryObj.getSaleListBySend(Integer.valueOf(iStart).intValue(),user_id);
  // ParamethodMgr paramObj=new ParamethodMgr();
  //HashMap typeMap=paramObj.getCompareInfoByCode("CRM","news_type");
    counter=enquiryObj.getSaleNumberBySend(user_id);
    }
String pageTools = tools.getPageTools(String.valueOf(counter), "30", "sendSaleIndex.jsp?&code="+code+"&key_word="+key_word+"&start_time="+start_time1+"&end_time"+end_time1+"&iStart=", Integer.parseInt(iStart));
%>
<html>
<head>
	  ...
	  %>
		  <div class="line6">
				 <div class="img"><img src="/images/icon3_manager_rightbody.gif" /></div>
				   <span class="title">您暂时还没有发表任何销售留言！</span><br />                  
		  </div>
	  <%
 }
%>
			
	<form action="sendSaleIndex.jsp" method="post" name="indexdateform">
		<%
				String top_menu_id="";
				if (request.getParameter("menu_id") != null){
	  ...
	  <tr>
	    <td>
		     <table width=800 border=0 cellpadding=2 cellspacing=1 align=center bgcolor="#e7e7e7">
		        <tr>
			        <td background="/images/newsbg.gif" style="color:#000000;  font-weight:bold; font-size:13px;" align=left width="25%">销售主题</td>
					<td background="/images/newsbg.gif" style="color:#000000;  font-weight:bold; font-size:13px;" align=center width="40%">留言内容</td>
					<td background="/images/newsbg.gif" style="color:#000000;  font-weight:bold; font-size:13px;" align=center width="10%">收言人</td>
					<td background="/images/newsbg.gif" style="color:#000000;  font-weight:bold; font-size:13px;" align=center width="15%">留言时间</td>
					<td background="/images/newsbg.gif" style="color:#000000;  font-weight:bold; font-size:13px;" align=center width="10%">处理状态</td>
		        </tr>
		        <%
