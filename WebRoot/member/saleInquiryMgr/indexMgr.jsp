...
<%
  ...
  if (logsession.getAttribute("SESSION_CUST_ID") != null) {
	...
	if("1".equals(code)){
		linkArray = enqBean.getEnquriyInfoByKey(Integer.parseInt(iStart), user_id, "7", "0",key_word);
		counter = enqBean.getEnquriyInfoNumberByKey(user_id, "7", "0",key_word);
	}else if("2".equals(code)){
		linkArray = enqBean.getEnquriyInfoByDate(Integer.parseInt(iStart), user_id, "7", "0",start_time1,end_time1);
		counter = enqBean.getEnquriyInfoNumberByDate(user_id, "7", "0",start_time1,end_time1);
	}else{
		linkArray = enqBean.getEnquriyInfoByTag(Integer.parseInt(iStart), user_id, "7", "0");
		counter = enqBean.getEnquriyInfoByTag(user_id, "7", "0");
	}
	String pageTools = tools.getPageTools(String.valueOf(counter), "20", "indexMgr.jsp?&code="+code+"&key_word="+key_word+"&start_time="+start_time1+"&end_time="+end_time1+"&iStart=",Integer.parseInt(iStart));
%>
<html>
	<head>
				...
				<td>
					<table width=100% border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#98D9A2">
						...
						<tr class="u2">
							...
							<td  align=center>
								<a href="replyEnquriy.jsp?trade_id=<%=trade_id%>"  onclick="mydefss()"><img src=/images/edit.gif width=16 height=16 border=0 alt="回复"> </a>
							</td>
						</tr>
						<%
						}
