...
String cust_id="",cust_name="";
   ...
   if (logsessionres.getAttribute("SESSION_CUST_ID") != null)
	...
	if("1".equals(code)){
		linkArray = enqBean.getEnquiryByMySelfKey(Integer.parseInt(iStart), user_id, "0",key_word);
		counter = enqBean.getEnquiryByMySelfKey(user_id, "0",key_word);
	}else if("2".equals(code)){
		linkArray = enqBean.getEnquiryByMySelfDate(Integer.parseInt(iStart), user_id, "0",start_time1,end_time1);
		counter = enqBean.getEnquiryByMySelfDate(user_id, "0",start_time1,end_time1);
	}else{
		linkArray = enqBean.getEnquiryByMySelf(Integer.parseInt(iStart), user_id, "0");
		counter = enqBean.getEnquiryByMySelf(user_id, "0");
	}
...
<html>
	...
	<form action="receiveList.jsp" method="post" name="indexdateform">
		...
		<table width=100% border="0" cellspacing="0" cellpadding="0" align=center>
			<tr>
				<td>
					<table width=100% border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#98D9A2D">
                ...
                String deal_tag=map2.get("deal_tag").toString();
                if(map2!=null){
                 if(map2.get("cust_name")!=null){
	                ...
	                if(map2.get("cust_id")!=null){
       				...
       				%>
						<tr class="u2">
							<td align="left">
								<a href="viewEnquiry.jsp?trade_id=<%=trade_id%>&cust_id=<%=qiuu_cust_id%>" ><%=title%></a>
							</td>
							<td align="left">
							    <%=enquiry_date%>
							</td>
							<td align="left">
							   <a href="/inc/include/InterimPages.jsp?cust_id=<%=qiuu_cust_id%>" TARGET=_blank><%=qiuu_cust_name%></a>
							</td>
							<td align="left">
							   <%=rsrv_str1%>
							</td>
							<td align=center>
								<a href="viewEnquiry.jsp?trade_id=<%=trade_id%>&cust_id=<%=qiuu_cust_id%>"  ><img src=/images/views.png width=16 height=16 border=0 alt="查看详细"> </a>
							</td>
						</tr>
						<%
						}
