...
<%
 ...
 //已推荐的
 ArrayList newsList =newsinfo.getNewsCommendList(Integer.parseInt(iStart),20,class_id);
  int counter=newsinfo.getNewsCommendList(class_id);
...
<body>
	<table width="800" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
				<td background="/admin/images/content_03.gif" height="43" width="156" align="center"><font size="2"><b>推荐资讯</b></font></td>
		 	  ...
		 	  <table width="100%" border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#DEEDFD">
		 		...
		 		<tr  class="u2"  >
					...
					<td>
						...
						<input name=info_type  type=hidden value="4" >
						<input name=news_type  type=hidden value="<%=class_id%>" >
	    ...
	    <td>
		     <table width="100%" border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#DEEDFD">
		        <tr class="u4" height="25">
					...
					<td width="15%" align=center>资讯分类</td>
		            ...
		            if(newsList != null && newsList.size()>0)
		            {
		            	 for (Iterator it = newsList.iterator(); it.hasNext();)
						  ...
						  String title="",news_id="",content="",publish_date="",publish_user_id="",user_name="",info_no="",custid="";
						      if (map.get("title") != null) {title = map.get("title").toString();}
						      if (map.get("news_type") != null) {class_id = map.get("news_type").toString();}

						       if (map.get("news_id") != null) {news_id = map.get("news_id").toString();}
						        if (map.get("content") != null) {content = map.get("content").toString();
						       if(content.length()>20) content=content.substring(0,19);}
						       
						       ArrayList classNamelist = new Productclass().genUpclassByClassId(class_id);
						    	if (classNamelist != null && classNamelist.size() > 0) 
						    	{
								 		HashMap claMap = (HashMap) classNamelist.get(0);
