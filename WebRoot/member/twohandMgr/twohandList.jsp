...
<%
	...
	if("1".equals(code)){
		saleList = saleObj.gentSalesByCust_idKey(Integer.valueOf(iStart).intValue(), cust_id,key_word,"1");
		counter = saleObj.getSaleListNumberByKey(cust_id,key_word,"1");
	}else if("2".equals(code)){
		saleList = saleObj.gentSalesByCust_idDate(Integer.valueOf(iStart).intValue(), cust_id,start_time1,end_time1,"1");
		counter = saleObj.getSaleListNumberByDate(cust_id,start_time1,end_time1,"1");
	}else{
		saleList = saleObj.gentSalesByMemeberCust_id(Integer.valueOf(iStart).intValue(), cust_id,"1");
		counter = saleObj.gentSalesByMemeberCust_id(cust_id,"1");
	}
	String pageTools=tools.getPageTools(String.valueOf(counter),"30","twohandList.jsp?&code="+code+"&key_word="+key_word+"&start_time="+start_time1+"&end_time="+end_time1+"&iStart=",Integer.parseInt(iStart));
%>
<html>
	<head>
	...
	<script language="javascript" src="/js/saleMgr.js"></script>
			<script language="javascript">
				...
				function reloadNews(val){
						...
						if(val==2){
							document.indexdateform.action='/doTradeReg.do';
							document.getElementById("trade_type_code").value="9234";
							document.indexdateform.submit();
						}
				}
	...
	<form action="twohandList.jsp" method="post" name="indexdateform">
	<%
			String top_menu_id="";
		...
		<table width=100% border="0" cellspacing="1" cellpadding="1" align="center">
				<tr class="u1">
					<td align="left"  class="head">
						<a href="addTwohandInfo.jsp">新增二手信息</a>
							</td>
				</tr>
		<table  width=100% border=0 cellpadding=1 cellspacing=1 bgcolor="#E6F6E9" align="center">
				<tr class="u2">
				<td >
		...
		<%if(null==saleList ) {%>
			<center>无二手记录！</center>
		<%} else{%>
		<table width=100% border="0" cellpadding="1" cellspacing="1" align=center bgcolor="#E6F6E9" style="margin-top:13px">
			...
			<tr class="u2">
				<td  align=left>
					<a href="viewtwoHandInfo.jsp?sale_id=<%=sale_id%>" title=<%=title%>><%=title%>
					</a>
				</td>
				<td  align=left>
					<%=start_date%>
				...
				<td   align=center>
					<a href=modifySaleInfo.jsp?sale_id=<%=sale_id%>><img src=/images/edit.gif width=16 height=16 border=0 alt="修改销售信息">
					</a>
				</td>
				<!--td  align=center>
					<a href="/doTradeReg.do?root_id=<%=sale_id%>&sale_id=<%=sale_id%>&info_id=<%=sale_id%>&trade_type_code=0349" target="_self" onClick="return chechIfo()"><img src=/images/del.gif width=16 height=16 border=0 alt="删除销售信息">
					</a>
				</td-->
