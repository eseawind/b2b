				...
				<td>
					<table width="100%" border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#DEEDFD">
						...
						<tr class="u2">
							...
							<td align=center>
								<a href="viewClassNewsInfo.jsp?class_id=<%=class_id%>&class_type=1" target=""><img src="/images/view.gif" border="0"></a>
							</td>
						</tr>
						<%
						}
						...
						<tr class="u3">
								...
								<a href="validityList.jsp?iStart=0&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">首页 </a>&nbsp; &nbsp;
								<% 
									if(Integer.parseInt(iStart)>0){
								%>
									<a href="validityList.jsp?iStart=<%=pageUp%>&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">上一页</a> &nbsp;
								<%
									}
									if(Integer.parseInt(iStart)<pages-1){
								%>
									<a href="validityList.jsp?iStart=<%=pageDown%>&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">下一页 </a>&nbsp; 
								<%
									}
								%>
								<a  href="validityList.jsp?iStart=<%=pages-1%>&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">尾页</a></td>
