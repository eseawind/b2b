...
<%
 request.setCharacterEncoding("gbk");
    ...
    if (!news_title.equals(""))
     {
		...
		//news_title = new String(news_title.getBytes("ISO8859_1"),"gbk");
	...
	String pageTools = tools.getPageTools(String.valueOf(counter),"20", "cancelCustomer.jsp?news_title="+news_title+"&iStart=", Integer.parseInt(iStart));
...
<script language="javascript">
	...
	<body>
		<form action="cancelCustomer.jsp" method="post" name="searchForm">	
		<table width="100%" border="0" cellpadding="1" cellspacing="1" align="left" bgcolor="#FCB0B0">
			<tr bgcolor="white">
					<td align="left" colspan="7">
					<font size="2"><b>	请输入公司名:</b></font>
			...
			<%
			    if (custArray != null && custArray.size() > 0) {
					...
					<tr class="u2"id="changcolor_tr<%=i%>" onMouseOver="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')"	onMouseOut="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')">
						...
						<td align=center><a href="/doTradeReg.do?cust_id=<%=cust_id%>&cust_state=2&trade_type_code=0267" target="_self" onClick="return chechIfo()"><img src=/images/delUser.png width=16	height=16 border=0 alt="冻结会员"></a></td>
					</tr>
					<%
					i++;
					}
					...
					<tr class="u1">
							...
							<a href="cancelCustomer.jsp?iStart=0&news_title=<%=news_title%>&i=1">首页 </a>&nbsp; &nbsp;
							<% 
								if(Integer.parseInt(iStart)>0){
							%>
								<a href="cancelCustomer.jsp?iStart=<%=pageUp%>&news_title=<%=news_title%>&i=1">上一页</a> &nbsp;
							<%
								}
								if(Integer.parseInt(iStart)<pages-1){
							%>
								<a href="cancelCustomer.jsp?iStart=<%=pageDown%>&news_title=<%=news_title%>&i=1">下一页 </a>&nbsp; 
							<%
								}
							%>
							<a  href="cancelCustomer.jsp?iStart=<%=pages-1%>&news_title=<%=news_title%>&i=1">尾页</a></td>
