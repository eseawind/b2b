...
<script language="javascript">
	...
	<body>
		...
		<table width="100%" border="0" cellpadding="1" cellspacing="1" align="center" bgcolor="#FCB0B0">
			...
			<%
			    if (custArray != null && custArray.size() > 0) {
					...
					<tr style="background-color:#f9f9f9; " id="changcolor_tr<%=i%>" onMouseOver="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#ffffff','DIV')"	onMouseOut="MM_changeProp('changcolor_tr<%=i%>','','backgroundColor','#f9f9f9','DIV')">
						...
						<td style=" color:#000000;" align=center><a href="/doTradeReg.do?cust_id=<%=cust_id%>&cust_state=0&trade_type_code=0167" target="_self" onClick="return chechIfo()"><img src=/images/defrost.gif width=16	height=16 border=0 alt="会员解冻"></a></td>
					</tr>
					<%
					i++;
					}
					...
					<tr class="u1">
							...
							<a href="defronstIndex.jsp?iStart=0&news_title=<%=news_title%>">首页 </a>&nbsp; &nbsp;
							<% 
								if(Integer.parseInt(iStart)>0){
							%>
								<a href="defronstIndex.jsp?iStart=<%=pageUp%>&news_title=<%=news_title%>">上一页</a> &nbsp;
							<%
								}
								if(Integer.parseInt(iStart)<pages-1){
							%>
								<a href="defronstIndex.jsp?iStart=<%=pageDown%>&news_title=<%=news_title%>">下一页 </a>&nbsp; 
							<%
								}
							%>
							<a  href="defronstIndex.jsp?iStart=<%=pages-1%>&news_title=<%=news_title%>">尾页</a></td>
			...
			{
			%>
				<tr><td align="center" colspan="5" bgcolor="white" style="font-weight:normal">无符合条件的记录！</td></tr>
			<%
			}
			%>
		</table></form>
