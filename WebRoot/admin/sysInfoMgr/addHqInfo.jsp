...
<%
	...
	ArrayList classlist = bean.getClassByUpClassId(Integer.parseInt(iStart),up_class_id,"12");
	int counter = bean.getClassByUpClassId(up_class_id,"12");
					...
					<table width="800" border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#DEEDFD">
						...
						<%	
								...
								if (classlist != null && classlist.size() > 0) {
									for (int a = 0; a < classlist.size(); a++) {
										...
										downList = bean.getClassInfoByUpClassId("12",class_id);
						...
						<tr class="u2">
							<td align=left>
								...
								%>
									<a href="modityNewsIndex.jsp?up_class_id=<%=class_id%>&menu_id=<%=top_menu_id%>"><%=class_name%></a>
								<%
									}else{
								%>
									<%=class_name%>
							...
							<td align=center>
								<a href="viewClassNewsInfo.jsp?class_id=<%=class_id%>&class_type=12" target="_blank"><img src="/images/view.gif" border="0"></a>
							</td>
							<td align=center>
								<a href="addNewsIndex.jsp?class_id=<%=class_id%>" target="_blank"><img src="/admin/images/xinzeng.png" border="0"></a>
							</td>
						</tr>
						<%
						}
						...
						<tr class="u3">
								...
								<a href="addHqIndex.jsp?iStart=0&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">首页 </a>&nbsp; &nbsp;
								<% 
									if(Integer.parseInt(iStart)>0){
								%>
									<a href="addHqIndex.jsp?iStart=<%=pageUp%>&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">上一页</a> &nbsp;
								<%
									}
									if(Integer.parseInt(iStart)<pages-1){
								%>
									<a href="addHqIndex.jsp?iStart=<%=pageDown%>&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">下一页 </a>&nbsp; 
								<%
									}
								%>
								<a  href="addHqIndex.jsp?iStart=<%=pages-1%>&menu_id=<%=top_menu_id%>&up_class_id=<%=up_class_id%>">尾页</a></td>
			
						</tr>
						<%
						}else{
						%>
						<tr align=center ><td colspan="5">无行情记录!</td></tr>
									  	<%}
						%>
					</table>
				</td>
