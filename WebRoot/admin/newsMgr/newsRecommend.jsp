					...
					<table width="100%" border=0 cellpadding=1 cellspacing=1 align=center bgcolor="#FCB0B0">
						...
						<tr class="u2">
							<td align=left>
								...
								%>
									<a href="newsRecommend.jsp?up_ch_id=<%=ch_id%>&menu_id=<%=top_menu_id%>">&nbsp;<%=ch_name%></a>
							...
							<td align=center>
							<%if(upp_ch_id.equals("0000000000") && downList!=null){%>
								<a href="newsRecommend.jsp?up_ch_id=<%=ch_id%>&menu_id=<%=top_menu_id%>"><img src="/images/view.gif" border="0"></a>
								<%}else{%>
								<a href="viewClassNewsInfo.jsp?class_id=<%=ch_id%>" target=""><img src="/images/view.gif" border="0"></a>
								<%}%>
							</td>
							<!--td align=center>
								<%
						...
						<tr class="u3">
								...
								<a href="newsRecommend.jsp?iStart=0&menu_id=<%=top_menu_id%>&up_ch_id=<%=upp_ch_id%>">首页 </a>&nbsp; &nbsp;
								<% 
									if(Integer.parseInt(iStart)>0){
								%>
									<a href="newsRecommend.jsp?iStart=<%=pageUp%>&menu_id=<%=top_menu_id%>&up_ch_id=<%=upp_ch_id%>">上一页</a> &nbsp;
								<%
									}
									if(Integer.parseInt(iStart)<pages-1){
								%>
									<a href="newsRecommend.jsp?iStart=<%=pageDown%>&menu_id=<%=top_menu_id%>&up_ch_id=<%=upp_ch_id%>">下一页 </a>&nbsp; 
								<%
									}
								%>
								<a  href="newsRecommend.jsp?iStart=<%=pages-1%>&menu_id=<%=top_menu_id%>&up_ch_id=<%=upp_ch_id%>">尾页</a></td>
